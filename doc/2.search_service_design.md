...
**`rerank(items: list[KnowledgeItem], query: str) -> list[KnowledgeItem]`**
...
**SDK 角色 boost（`KnowledgeMeta.role`）：**
...
**关键词命中加成**：每个命中关键词 +0.02。

**公式**：`final_score = item.score * type_boost * role_boost + keyword_hit * 0.02`
