...
| v1.2 | 2026-05-10 | 引入Search Service微服务架构，新增auto_assemble参数、Remote引擎切换 |
...
| `knowledge_types` | string[] | 否 | 过滤知识类型。可选值：`term`, `api`, `best_practice`, `defect_history`, `security_rule`, `test_template`。 |
...
### 6.1 概述
将多来源知识（检索结果、规范契约、预加载知识）经过安全清洗、去重、裁剪后，组装为结构化的4段式Prompt。支持自动注入版本约束、安全要求等生成条件。
...
```
输入 → 安全清洗 → 去重 → 分组排序 → Token裁剪 → 4段式组装 → 输出
```
