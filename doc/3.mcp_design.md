...
**检索结果按来源分组（v1.3 新增）**：
- SDK 源码（`knowledge_source: "sdk_code"`）：方法签名精确可靠，优先展示在 Background 的「方法签名」子节。每条 SDK 条目显式标注 `import:` 路径、`返回类型:` 和 `SDK 版本:`，消除 LLM 的 import 幻觉和类型误认（v1.5）
...
```json
{
  ...
  "sections": {
    ...
    "background": "以下是从私域知识库检索到的相关知识...\n\n### 方法签名（SDK 源码）\n### [API] [SDK源码] (score: 0.96)\nimport: com.example.sdk.PointsClient;\n返回类型: String\nSDK 版本: 2.3.0\n\nimport com.example.sdk.PointsClient;\nPointsClient.deduct(bizId, uid, points) → String\n\n### 参考文档\n### [API] [文档] (score: 0.96)\n积分扣减 API 说明...",
    "user_request": "## 用户需求\n{user_query}",
    "constraints": "## 生成约束\n- 版本要求: v2.3..."
  },
  "stats": {
