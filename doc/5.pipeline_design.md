...
`code_parser.py` 的 `_detect_class_role()` 根据包路径 + 方法特征自动标注：

| role | 判定条件 | 示例 |
|------|----------|------|
| `entry_point` | 根包或 service 子包 + 含静态工厂方法（static + 返回非基本类型） | `FileServiceManager.getTenantService()` |
| `public_api` | service/api/client 子包，或根包的接口/抽象类 | `FileService.uploadFile()` |
| `internal` | dao/config/impl/internal 子包，或全是 getter/setter | `LocalFileStorageDao.upload()` |
